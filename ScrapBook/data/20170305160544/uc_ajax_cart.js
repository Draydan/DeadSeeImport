...
return cookieValue;}};Drupal.behaviors.ucAjaxCart=function(context)
{jQuery.blockUI.defaults.growlCSS.opacity=1;jQuery.blockUI.defaults.timeout=Drupal.settings.uc_ajax_cart.TIMEOUT;ajaxCartCheckCartToggle();jQuery('a.ajax-cart-link').bind('click',function(){ajaxCartBlockUI(Drupal.settings.uc_ajax_cart.ADD_TITLE,'<div class="messages status one">THREE'+ Drupal.settings.uc_ajax_cart.ADD_MESSAGE+'</div>')
jQuery.get(Drupal.settings.uc_ajax_cart.CART_LINK_CALLBACK,{href:this.href},ajaxCartFormSubmitted);return false;});jQuery('form.ajax-cart-submit-form input.ajax-cart-submit-form-button').not('.ajax-cart-processed,#edit-update').click(function(){var form=jQuery(this).parents('form').eq(0);form.validate();if(form.valid()==false){return false;}
form.ajaxSubmit({url:Drupal.settings.uc_ajax_cart.CALLBACK,beforeSubmit:function(){ajaxCartBlockUI(Drupal.settings.uc_ajax_cart.ADD_TITLE,'<div class="messages status two">'+ Drupal.settings.uc_ajax_cart.ADD_MESSAGE+'</div>')},success:ajaxCartFormSubmitted,type:'post'});return false;}).addClass('ajax-cart-processed');if(Drupal.settings.uc_ajax_cart.CART_VIEW_ON)
...
{jQuery('#uc-cart-view-form #edit-update').bind('click',function(){jQuery(this).parents('form').ajaxSubmit({url:Drupal.settings.uc_ajax_cart.UPDATE_CALLBACK,success:ajaxCartUpdateCartView,beforeSubmit:function(){jQuery('#uc-cart-view-form input').attr('disabled','disabled');ajaxCartBlockUI(Drupal.settings.uc_ajax_cart.ADD_TITLE,'<div class="messages status three">TWO'+ Drupal.settings.uc_ajax_cart.UPDATE_MESSAGE+'</div>')}});return false;});jQuery('#uc-cart-view-form input').bind('change',function(){jQuery('#uc-cart-view-form #edit-update').trigger('click');});}
function ajaxCartCheckCartToggle()
{if(jQuery.cookie('ajax-cart-visible')=='1')
{jQuery('#ajaxCartUpdate #cart-block-contents-ajax').show();}else{jQuery('#ajaxCartUpdate #cart-block-contents-ajax').hide();}}
